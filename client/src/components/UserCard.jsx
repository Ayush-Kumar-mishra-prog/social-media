...
const UserCard = ({user}) => {
  ...
  return (
    <div className='p-4 pt-6 flex flex-col justify-center w-72 shadow border border-gray-200 rounded-md 'key={user._id}>
   ...
   <div className="flex items-center justify-center gap-2 mt-4 text-xs text-gray-600">
    <div className="flex items-center gap-1 brder border-gray-300 rounded-full px-3 py-1">
   ...
   <div className="flex mt-4 gap-2">
    ...
    <button onClick={handleFlow} disabled = {currentUser?.following.includes(user._id)} className="w-full py-2 rounded-md flex justify-center items-center gap-2 bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 active:scale-95 transition text-white cursor-pointer">
        <UserPlus className='w-4 h-4'/> {currentUser?.following.includes(user._id) ? 'Following' : 'Follow'}
    </button>

    {/* Connectin request button */}
